...
<head>
  ...
  <script src="/js/adminJS/adminBoardList.js" defer></script>
  <title>게시판 목록</title>
</head>
<body>
<div class="wrap">
  ...
  <div class="main">
    ...
    <div align="center" class="paging">
      <a href="<c:url value="/admin?action=boardList&pageNum=1"/>"><span>첫페이지</span></a>
      <%
        if(thisBlock > 1) {
      %>
      <a href="/admin?action=boardList&pageNum=<%=(firstPage - 1)%>"><span> < 이전 </span></a>
      <%
        }
      %>
      ...
      %>
      <a href="/admin?action=boardList&pageNum=<%=(lastPage + 1)%>"><span> 다음 ></span> </a>
      <%
        }
      %>
      <a href="<c:url value="/admin?action=boardList&pageNum=${totalPage}"/>"><span>끝페이지</span></a>
    </div>
