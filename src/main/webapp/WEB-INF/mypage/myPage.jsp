                  ...
                  <label for="chk${reviewDTO.movieNo}">
                    ...
                    <a href="view.movie?action=view&movieNo=${reviewDTO.movieNo}" class="text-decoration-none" >
                      <p class="myList"> ${reviewDTO.review}
                          ${reviewDTO.addDate}
                          ${commentDTO.nickName}
                        <a href="./remove.movie?action=remove&movieNo=${reviewDTO.movieNo}" onclick="return confirm('정말 삭제하시겠습니까?');"
                           class="remove-btn" style="font-size: 30px">X</a>
                      </p>
                    </a>
                  </div>
...
function removeAll(tabId) {
  const selectedItems = document.querySelectorAll(`.${tabId} input[name="selectedItems"]`);
  if (confirm('정말 삭제하시겠습니까?')) {
    selectedItems.forEach(item => {
      item.checked = false; // 전체 삭제 선택
    });

    // You can submit the form here or handle the removal action
    // Update the form action URL accordingly
    const frm = document.querySelector(`form[name="frmRemoveSelected${tabId.charAt(tabId.length - 1)}"]`);
    frm.submit();
  }
}

// Function to handle "선택 삭제" for each tab
function removeSelected(tabId) {
  const selectedItems = document.querySelectorAll(`.${tabId} input[name="selectedItems"]:checked`);
  if (confirm('정말 삭제하시겠습니까?')) {
    const frm = document.querySelector(`form[name="frmRemoveSelected${tabId.charAt(tabId.length - 1)}"]`);
    frm.submit();
  }
...
// "전체 삭제" and "선택 삭제" buttons in each tab
const tabIds = ["tab1", "tab2", "tab3"]; // Add more tabIds if needed

tabIds.forEach(tabId => {
  const btnRemoveAll = document.querySelector(`.${tabId} .btn-removeAll`);
  const btnRemoveSelected = document.querySelector(`.${tabId} .btn-selected${tabId.charAt(tabId.length - 1)}`);
